...
def filecount(i):
    """Creates a counter that upticks when a sequentially named image is found. This counter tracks how many images we are matching stored in 'filecount'"""
    i=1
    pathPresence = True
...
#-----------------------------------------------
def create_dict(i):
    """Creates a dictionary to hold sample images for looping through. The length of the dictionary is also the new filecount variable"""
...
def create_template ():
    """Loads 9 reference image files, labeled by bregma coordinates, into a 3x3 single grid. The grid is stored and used as the reference for the sample images"""
...
def match_images(i):
    """Creates a loop that runs the length of imgDict and creates a copy of template_grid each time to draw rectangle on. The output name is the input name minus img"""
